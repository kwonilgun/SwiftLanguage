...
extension Sendable {
    ...
    func send(data: Any) {
        ...
        print("self.to:\(receiver.nameR ?? "nil")")
        print("self.from:\(self.from.nameS ?? "nil")")
...
class Message: Sendable, Receivable {
    var nameS: String? = "Send"
    let nameR: String? = "Receive"
    var to: Receivable?
...
class Mail: Sendable, Receivable {
    let nameR: String? = nil
    let nameS: String? = "Sun"
    var to: Receivable?
}
